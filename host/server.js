...
viewmodel.read(options.repository, function(err, repository) {
    ...
    eventDenormalizer.init(function(err) {
        ...
        console.log(k.collections[0].viewBuilders);
        console.log(k.collections[1].viewBuilders);
        ...
        msgbus.onEvent(function(data) {
            ...
            eventDenormalizer.handle(data,function(err){
                if(err)
                {
                        console.log(colors.red('YOO ERROR FROM EVENT DENORMALIZER : '+err))
                }
            });
        });
